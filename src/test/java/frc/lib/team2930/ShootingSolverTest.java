...
public class ShootingSolverTest {
  ...
  public void testOrbit() {
    Translation3d Ptarget = new Translation3d(0, 0, 10);
    ...
    //
    // Create a trajectory tangential to the target, rotating it around the target,
    // testing different shooter speeds.
    //
    for (double shooterSpeed = 10; shooterSpeed < 60; shooterSpeed += 10) {
      var solver =
          new ShootingSolver(
              Ptarget, PaxisOfRotationShooter, PfrontOfShooter, shooterSpeed, shootingTime, true);
      ...
      for (double angle = 0; angle < 360; angle += 45) {
        ...
        double VgamepieceHorizontal = shooterSpeed * Math.cos(Math.toRadians(45));
        if (VgamepieceHorizontal < 10) {
          assertNull(res);
        } else {
          ...
          var gamepieceStart3d = new Translation3d(shooterSpeed, 0, 0);

          var gamepieceMid3d =
              gamepieceStart3d.rotateBy(new Rotation3d(0, -res.pitch().getRadians(), 0));

          var gamepieceVel3d =
              gamepieceMid3d.rotateBy(new Rotation3d(0, 0, res.heading().getRadians() + Math.PI));

          var robotVel3d = GeometryUtil.translation2dTo3d(robotVelRotated);

          var gamepieceVelTotal = robotVel3d.plus(gamepieceVel3d);

          if (debugCode) {
            System.out.printf("gamepieceStart3d: %s\n", gamepieceStart3d);
            System.out.printf("gamepieceMid3d: %s\n", gamepieceMid3d);
            System.out.printf("gamepieceVel3d: %s\n", gamepieceVel3d);
            System.out.printf("robotVel3d: %s\n", robotVel3d);
            System.out.printf("gamepieceVelTotal: %s\n", gamepieceVelTotal);
            System.out.printf("totalSpeed: %.1f\n", gamepieceVelTotal.getNorm());
            System.out.printf(
                "heading: %.1f\n",
                Math.toDegrees(Math.atan2(gamepieceVelTotal.getY(), gamepieceVelTotal.getX())));
          }

          var gamepieceHorizontalReal =
              Math.hypot(gamepieceVelTotal.getX(), gamepieceVelTotal.getY());
          var gamepieceVerticalReal = gamepieceVelTotal.getZ();
          assertEquals(
              45, Math.toDegrees(Math.atan2(gamepieceVerticalReal, gamepieceHorizontalReal)), 0.1);
        }
      }
    }
  }
